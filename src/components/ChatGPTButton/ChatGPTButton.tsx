...
const ChatGPTButton = ({ setDescription }: props) => {
	...
	const [askChatGPT, { data, loading }] = useLazyQuery<AskChatGptQuery, AskChatGptQueryVariables>(
		GET_DESCRIPTION_FROM_CHATGPT
	);
			...
			<Modal open={open} onClose={() => setOpen(false)} closeAfterTransition id='chatGPT-modal'>
				<Fade in={open}>
					<div id='chatGPT-form'>
						...
						/>
						{loading ? <Spinner /> : <button onClick={getDescription}>Få en beskrivelse</button>}
